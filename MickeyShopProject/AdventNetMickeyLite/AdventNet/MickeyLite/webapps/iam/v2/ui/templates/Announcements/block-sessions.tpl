...
<script>
	...
	function terminateAllSession(){
	    ...
	    var objHTTP = xhr();
	    objHTTP.open('DELETE', "${za.contextpath}"+"/webclient/v1/announcement/pre/blocksessions", true);
		    ...
		    if(objHTTP.readyState==4) 
		    {
				...
				else
				{
					...
					{				
						...
						document.getElementById("continue_button").classList.remove("loading_btn");
						document.getElementById("continue_button").setAttribute("onclick", "terminateAllSession()");
						if(jsonStr.code === "Z113"){
							showErrMsg("<@i18n key="IAM.ERROR.SESSION.EXPIRED"/>");
						}
						else if(jsonStr.status_code.toString() === "204"){
							window.location.href = "${Encoder.encodeJavaScript(nxt_preann_url)}";
						}
						else{
