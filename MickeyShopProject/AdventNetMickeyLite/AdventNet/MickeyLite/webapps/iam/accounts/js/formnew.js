...
function Form($form, options) {
	...
	_this.element.submit(function(event) {
		if ($(".form").attr("submitted") && $(".form").attr("submitted") === 'true'){
			typeof validateOTP == "function" ? validateOTP() : ""; // no i18n
			return false;
		}
		_this.submit(event);
	});
...
Form.prototype.submit = function(event) {
	...
	_this.element.parent().find(".field-msg").remove();
...
Form.Message = {
	...
	error : function(selector, msg) {
		try{
			var $input = $("input[name='"+selector+"']").length === 0 ? $("form[name='"+selector+"']") : $("input[name='"+selector+"']");
		}catch(e){
			$input = $(selector);
		}
		if (msg) {
	...
	create : function($input, type, msg) {
		...
		if (!$fieldMsg.length) {
			$fieldMsg = $("<div class='field-msg'></div>")[isForm ? "prependTo" : "appendTo"]($input.parent()); // No I18N
		}
