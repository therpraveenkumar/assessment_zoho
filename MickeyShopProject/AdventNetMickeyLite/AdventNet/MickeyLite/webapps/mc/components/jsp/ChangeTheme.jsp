			...
			<%
				...
				while(iterator.hasNext()){
					...
					if(themeName.equals(currentTheme)){
						%>
						<td align="center" class="bodyText"><img src='<%out.print(IAMEncoder.encodeHTMLAttribute(icon));//NO OUTPUTENCODING%>'><br><input type="radio" name="selectedTheme" value="<%out.print(IAMEncoder.encodeHTMLAttribute(themeName));//NO OUTPUTENCODING%>" checked onClick="return changeTheme()">&nbsp;<%out.print(IAMEncoder.encodeHTML(I18N.getMsg(themeName)));//NO OUTPUTENCODING%></td><%--NO OUTPUTENCODING --%>
					<%
					}
					else {
						%>
						<td align="center" class="bodyText"><img src='<%=IAMEncoder.encodeHTMLAttribute(icon)%>' onClick="return applyTheme('<%=IAMEncoder.encodeJavaScript(themeName)%>')"><br><input type="radio" name="selectedTheme" value="<%=IAMEncoder.encodeHTMLAttribute(themeName)%>" onClick="return changeTheme()">&nbsp;<%=IAMEncoder.encodeHTML(I18N.getMsg(themeName))%></td>
					<%
					}
				}
			%>
